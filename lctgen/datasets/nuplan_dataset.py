...
class NuplanDataset(Dataset):
    ...
    def nuplan_process(self, data, index):
        ...
        all_agents = self.extract_agents(self.tracks)
        ...
        future_attrs = ['gt_pos', 'gt_vec_index', 'traj', 'future_heading', 'future_vel', 'gt_agent', 'gt_agent_mask', 'all_agent']
    ...
    def extract_agents(self, tracks):
        type_vehicle = {"VEHICLE": 0, "TRAFFIC_CONE": 3, "PEDESTRIAN": 1, "CYCLIST": 2, "TRAFFIC_BARRIER": 4}

        num_agents = len(tracks)
        ego_track = tracks["ego"]
        time_steps = ego_track["state"]["position"].shape[0]
        all_tracks = np.zeros((num_agents, time_steps, 9))
        
        all_tracks[0, :, :2] = ego_track["state"]["position"][:, :2]
        all_tracks[0, :, 2:4] = ego_track["state"]["velocity"]
        all_tracks[0, :, 4] = ego_track["state"]["heading"]
        all_tracks[0, :, 5] = ego_track["state"]["length"][:, 0]
        all_tracks[0, :, 6] = ego_track["state"]["width"][:, 0]
        all_tracks[0, :, 7] = type_vehicle[ego_track[SD.TYPE]]
        all_tracks[0, :, 8] = ego_track["state"]["valid"]
        ...
        for id, track in tracks.items():
            if id == "ego":
                continue
            all_tracks[idx+1, :, :2] = track["state"]["position"][:, :2]
            all_tracks[idx+1, :, 2:4] = track["state"]["velocity"]
            all_tracks[idx+1, :, 4] = track["state"]["heading"]
            all_tracks[idx+1, :, 5] = track["state"]["length"][:, 0] 
            all_tracks[idx+1, :, 6] = track["state"]["width"][:, 0] 
            all_tracks[idx+1, :, 7] = type_vehicle[track[SD.TYPE]]
            all_tracks[idx+1, :, 8] = track["state"]["valid"]
            idx += 1


            if track[SD.TYPE] in ["TRAFFIC_CONE", "TRAFFIC_BARRIER", "PEDESTRIAN", "CYCLIST"]:
            ...
            '''
                ...
                # all_tracks[idx, :, 5] *= extension_rate
                # all_tracks[idx, :, 6] *= extension_rate
                if all_tracks[idx, -1, 0] < 0.5 and all_tracks[idx, -1, 1] < 0.5:
                    rows_to_del.append(idx)
            ''' 
