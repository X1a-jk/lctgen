...
def gen_scenario_from_gpt_text(llm_text, cfg, model, map_vecs, map_ids):
    ...
    for k in batch.keys():
        if type(batch[k])==torch.Tensor:
            batch[k] = batch[k].cpu()
    for q in output_scene[0]:
        if type(output_scene[0][q])==torch.Tensor:
            output_scene[0][q] = output_scene[0][q].cpu()
...
query = 'The car makes a left lane change.'  # @param {type:"string"}
